...
//update DOM elements for the current product (on detail page)
...
// update cart array when new roll is added (on detail page)
function updateCart(){
...
function initializeCart(rollsArray){    
...
function saveToLocalStorage(){
    let rollsJSON = JSON.stringify(cart);
    localStorage.setItem("storedRolls",rollsJSON);
    console.log("local storage is updated:"+rollsJSON);
}

function retrieveFromLocalStorage(){
    let rollsJSON = localStorage.getItem("storedRolls");
    if (rollsJSON == null){
        cart = [];
        return;
    }
    //put all rolls into cart array
    let rollsArray = JSON.parse(rollsJSON);

    for (let rollData of rollsArray){
        let roll = new Roll(rollData.type, rollData.glazing, rollData.size, rollData.basePrice);
        cart.push(roll)
    }
    console.log("data is retrieved from local storage:"+rollsJSON);
...
retrieveFromLocalStorage(); //when initializing the page, retrieve roll data from local storage
